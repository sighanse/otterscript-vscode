...
- Diagnostics for common mistakes (missing `$`, unknown functions, invalid operators, `=` used in `if` conditions, duplicate map keys, unknown `Namespace::` prefixes, malformed `<% %>` text-template tags, template/expression mode mixing)
